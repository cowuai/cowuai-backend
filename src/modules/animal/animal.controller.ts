...
    }
    ...
    // =========================================================
    // MODIFICAÇÃO: Implementação da Paginação no findAll
    // =========================================================
...
                 return res.status(400).json({ error: "Parâmetros de paginação (page, pageSize) devem ser positivos." });
            }

            // 2. Chamar o serviço com os parâmetros de paginação
            // Assumimos que o service.findAllPaginated retornará { animals, total }
            const { animals, total } = await this.animalService.findAllPaginated(page, pageSize);
...
            });
        } catch (error) {
            // O parâmetro 'req' agora é usado, eliminando o '_req'
            errorHandler(error as Error, req, res, () => {});
        }
    }
    // =========================================================
